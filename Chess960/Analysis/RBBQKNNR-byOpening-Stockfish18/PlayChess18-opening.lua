...
if #arg >= 1 then
  opening = arg[1]
  if opening == "--help" or opening == "-help" or opening == "help" then
    print("Usage: PlayChess18-opening.lua {opening} {plies} {setup} {seed}")
    print("Example: PlayChess18-opening.lua 'e2e4 e7e5 f2f4' 18")
    os.exit(0)
  end
end
if #arg >= 2 then
...
if #arg >= 4 then
  gSeed = arg[4] -- Yes, seeds can be strings (with Lunacy)
end
...
while true do
  ...
  if fields[1] == "bestmove" then
    ...
    if(move:match('none')) then
      ...
      w:write("quit\n") io.flush() -- Let’s have a clean exit
      os.exit(0)
    end
