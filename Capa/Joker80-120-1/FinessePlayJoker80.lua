...
minutesPerNMoves = 1
if #arg >= 3 then
  minutesPerNMoves = tonumber(arg[3])
  if minutesPerNMoves < 1 then minutesPerNMoves = 1 end
end
...
params = {
  ...
  -- variantFEN = "rnabckbqnr/pppppppppp/10/10/10/10/PPPPPPPPPP/RNABCKBQNR " ..
  --          "w KQkq - 0 1",
...
if not MultiPV or MultiPV < 2 then
  print("MultiPV too small/not set, using 1")
  MultiPV = 1
end
...
-- w:write("sd 7\n") -- Depth 7 moves
w:write("level " .. NMoves .. " " .. minutesPerNMoves .. " 0\n") -- Set time
w:write("p\n") -- Joker80 uses “p”, not “d” like (fairy-)stockfish
w:flush()
lineFromEngine = ""
while not string.match(lineFromEngine,'GameEval') do -- Different p output
...
while true do
  lineFromEngine = r:read()
  if not lineFromEngine then
    os.exit(0)
  end
  if lineFromEngine:match('%{') then
    print(lineFromEngine)
    os.exit(0)
  end
  print(lineFromEngine)
  if lineFromEngine:match('^move') then
    w:write("go\n")
    w:flush()
  end
end
