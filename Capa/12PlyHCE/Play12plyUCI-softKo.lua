...
-- a semi-Ko rule: If a given position has been seen before (where the
...
if rg32 == nil then
  print("rg32 Random number library is required\n")
  print("Download lunacy at https://github.com/samboy/lunacy or get libs\n")
  print("(rg32 and spawner) at https://github.com/samboy/LUAlibs\n")
  os.exit(1)
end
...
function processFENline(hash, line)
  ...
  if not hash[line] then
    ...
    if hash[line] > 1 and not isForcedKo then 
  ...
  if fields[1] == "bestmove" then
    ...
    for k,v in sPairs(multiMoves) do 
      if type(v) == 'table' and v.v then
        -- Semi-Ko rule: We don’t consider winning moves we have already
    ...
    isForcedKo = false
    if forcedKo[toConsider] then isForcedKo = true end
    forcedKo = {}
